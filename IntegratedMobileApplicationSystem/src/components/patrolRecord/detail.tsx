...
export default class PatrolRecordInformation extends React.Component<any, any>{
    ...
    async componentDidMount() {
        ...
        new Http().doGet('check/selectCheckUW?id=' + data.id, null, null, null)
            .then((e) => {
        ...
        new Http().doGet('check/listUI?checkId=' + data.id, null, null, null)
            .then((e) => {
                const data = e.data
                let subContent = []
                let list = []
                ...
                for (let item of data) {
                    subContent.push({ id: item.id, jieguo: item.resultJie, shuo: item.explan, shijian: item.threId, userName: JSON.parse(user).user.loginCode })
                ...
                new Http().doPost('file/huoqu', null, { list: list }, null)
                    .then((e) => {

                        for (var item of e.data) {
                            for (let _item of picturesList) {
                                if (item.pid === _item.id) {
                                    _item.path = item.list
                                }
                            }
                            for (let _item of imageList) {
                                if (item.pid === _item.id) {
                                    _item.base64 = item.list
                                }
                            }
                        }
                        this.setState({
        ...
        addLocationListener(location => {
            if (location.errorInfo === "success") {
                this.caculateLL(location.longitude, location.latitude, Number(data.longitude).toFixed(4), Number(data.latitude).toFixed(4))
                stop()
            }
        });
    }
    ...
    // 上传
    send() {
        const { data, user, imageList, status } = this.state
        const { checkDescription, checkResultDescription, } = this.params
        if (+data.checkDeviation < this.state.s) {
    ...
    //上传发送
    async submit() {
        const { user, subContent, imageList, trunEvent } = this.state;
        this.refs.loading['show']();
        for (let item of subContent) {
            if (item.jieguo === '转事件' && !item.shijian) {
        ...
        await new Http().doPost('check/xiuSave', null,
            { list: this.state.subContent }, null)
            .then((e) => {
                const data = e.data
                let list = []
                data.map((item, index) =>
                    list.push({ list: imageList[index].base64, userCode: user.userCode, userName: user.userName, pid: item })
                )
                new Http().doPost('file/uploadToK', null, {
                    list
                }, null).then((e) => {
                    if (e.data.code === 20000) {
                        Alert.alert(null, '修改成功', [{ text: "确定", onPress: () => { this.props.navigation.goBack(); } }])
                        this.refs.loading['hide']();
                    }
                }).catch(e => {
                    Alert.alert(null, '修改失败', [{ text: '确认' }])
                    this.refs.loading['hide']();
                })
            }).catch((e) => {
                Alert.alert(null, '修改失败', [{ text: '确认' }])
                    ...
                    <View style={styles.header}>
                        <View style={styles.headerTitle}>
                            ...
                            <View style={{ flexDirection: 'row' }}>
                                <Button style={[styles.buttonStyke, { marginRight: 10 }]} onPress={() => { this.submit() }}>修改</Button>
                            </View>
                        </View>
                        <View style={styles.headerContainer}>
                            <View style={{ width: '100%', marginBottom: 10 }}><Text style={{ fontSize: 16 }}>巡检点ID：<Text style={{ color: "#2a5696", fontSize: 16 }}>{data.bianma}</Text></Text></View>
                            <View style={{ width: "100%", flexDirection: 'row' }}>
                                <View style={{ flex: 1 }}>
                                    <View style={styles.headerContainerTitle}>
                                        <Text style={{ fontSize: 16 }}>所属机构：</Text>
                                        <View style={{ flex: 1 }}>
                                            <Text style={{ color: "#2a5696", fontSize: 16 }}>{data.checkCenter}</Text>
                                        </View>
                                    </View>
                                    <View style={styles.headerContainerTitle}>
                                    ...
                                    <View style={styles.headerContainerTitle}>
                                        ...
                                        <View style={{ flex: 1 }}>
                                            <Text style={{ color: "#2a5696", fontSize: 16 }}>{data.mark + ";" + data.teMark}</Text>
                                        </View>
                                    </View>
                                </View>
                                <View style={{ flex: 1 }}>
                                    <View style={styles.headerContainerTitle}>
                                        ...
                                        <View style={{ flex: 1 }}>
                                            <Text style={{ color: "#2a5696", fontSize: 16 }}>{Number(data.longitude).toFixed(4)}</Text>
                                        </View>
                                    </View>
                                    <View style={styles.headerContainerTitle}>
                                        <Text style={{ fontSize: 16 }}>纬度：</Text>
                                        <View style={{ flex: 1 }}>
                                            <Text style={{ color: "#2a5696", fontSize: 16 }}>{Number(data.latitude).toFixed(4)}</Text>
                                        </View>
                                    </View>
                                </View>
                            </View>
                            <View style={{ width: '100%', marginBottom: 10 }}><Text style={{ fontSize: 16 }}>提交时间:<Text style={{ color: '#2a5696' }}>{data.updateDate}</Text></Text></View>
                        </View>
                    </View>
                    {
                                ...
                                <View key={index} style={styles.submitInfo}>
                                    ...
                                    <View style={styles.headerContainer}>
                                        ...
                                        <View style={styles.submitItem}>
                                            ...
                                            {
                                                item.resultJie === '转事件' ? <Button style={styles.buttonStykeTrue}>转事件</Button> :
                                        ...
                                        <View>
                                            ...
                                            <View style={styles.submitItem}>
                                                ...
                                                <View style={styles.s_box} onLayout={(e) => { this._onLayout(e) }}>
                                                    {
                                                        picturesList[index] && picturesList[index].path.map((_item, _index) => (
                                                            <TouchableOpacity key={_index} onLongPress={() => { this.removeImg(item.id, _index) }}>
