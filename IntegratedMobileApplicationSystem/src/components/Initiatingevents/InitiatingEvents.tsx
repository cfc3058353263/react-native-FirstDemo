 ...
 * 发起事件
 */
export default class InitiatingEvents extends React.Component<any, any>{
    ...
    async submit(): Promise<void> {
        ...
        if (pictures.length !== 0) {
            ...
            for (let item of pictures) {
                await ImageResizer.createResizedImage(item, 500, 500, "JPEG", 70)
                    .then(response => {
            ...
            await RNFetchBlob.fetch('POST', 'http://124.128.244.106:9100/api-system/system/file/fileUpload', {
                ...
                if (data.code === 0) {
                    this.submitData['common_id'] = data.result.common_id
                    this.getAddEvent(this.submitData)
                }
            }).catch((err) => {
                this.refs.loading['hide']();
                Alert.alert(null, "上传图片过多，图书事件发起失败")
            })
        } else {
            this.getAddEvent(this.submitData)
        }
    }
    getAddEvent(submitData) {
        new Http().setToken(this.state.token).doPost("api-yjdd/yjdd/eventLog/addEventLog", null, JSON.stringify(submitData), null).then((e: any) => {
            this.refs.loading['hide']();
            if (e.data.code === 0) {
                Alert.alert(null, "事件发起成功", [{ text: "确定" }]);
            } else {
            ...
            if (images) {
                for (let item of images) {
                    this.setState({
                        ...
                        for (let item of this.state.pictures) {
                            this.setState({
                                imageShowList: [...this.state.imageShowList, { url: item }]
                            })
                        }
                    })
                }
  ...
  */
    ...
    render(): React.ReactElement<any, string | React.JSXElementConstructor<any>> | string | number | {} | React.ReactNodeArray | React.ReactPortal | boolean | null | undefined {
        ...
        const { query, scrollEnabled, pictures, focus } = this.state;
        ...
        return (
            ...
            >
                ...
                <View style={styles.view}>
                    ...
                    <View style={{ marginBottom: 10 }}>
                        <ModuleInput context={"事件名称"} onChangeText={(txt: string, step: number) => { this.input(txt, "event_name"); }} />
                    </View>
                    <View style={{ marginBottom: 10 }}>
                        <ModuleSelect context={"事件类型"} getWidth={(e) => this.getWidth(e.nativeEvent.layout.width)} data={['管道暗渠故障', '溃提或滑坡事件', '渡槽倒虹吸突发事件', '坠渠救助', '其他']} onSelect={(index, value) => { this.onselect(index, value) }} disabled={false} />
                    </View>
                    <View style={{ marginBottom: 10 }}>
                        <ModuleInput context={"地址"} width={this.textWidth} spaceBetween={true} onChangeText={(txt: string, step: number) => { this.input(txt, "addr"); }} />
                    </View>
                    <View style={{ marginBottom: 10 }}>
                    ...
                    <View style={{ marginBottom: 10 }}>
                        <ModuleInput context={"事件内容"} onChangeText={(txt: string, step: number) => { this.input(txt, "content"); }} />
                    </View>
                    <View style={{ marginBottom: 10 }}>
                            ...
                            <View style={styles.s_box} onLayout={(e) => { this._onLayout(e) }}>
                                {
                                    pictures.length !== 0 && pictures.map((item, index) => (
                                        <TouchableOpacity key={index} onLongPress={() => { this.removeImg(index) }} onPress={() => { this.setState({ isImageShow: true, imgIdx: index }) }} >
